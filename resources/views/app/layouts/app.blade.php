	...
	@php
		...
		try {
			...
			if ($authUser) {
				...
				if ($tenantId) {
					$tenantSetting = \App\Models\Setting::where('tenant_id', $tenantId)->first();
					if ($tenantSetting && $tenantSetting->company) {
						$appTitle = $tenantSetting->company . ' | Management System';
					}
				}
			}
		} catch (\Throwable $e) {
			// title hesaplanamazsa config('app.name') kullanılacak
		}
	@endphp

	<title>{{ $appTitle }}@hasSection('title') | @yield('title')@endif</title>
